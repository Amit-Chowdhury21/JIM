...
with open(r"e:\PRO\JIMxNik\jim_new\src\models\tft_pro_max.py", "w", encoding="utf-8") as f:
    f.write('''"""
TFT_Pro_Max_V4 — Institutional Production Temporal Fusion Transformer
====================================================================
...
# COST MODELING
# ============================================================================
def effective_cost(static_cost: float, gvz: float, session_overlap_flag: float) -> float:
    """Scale execution cost if volatility (gvz) is extremely high or during high-volume overlap."""
    vol_factor = 1.0 + 0.5 * max(0, gvz - 10) / 10.0
    session_factor = 1.2 if session_overlap_flag > 0.5 else 1.0
    return static_cost * vol_factor * session_factor
...
def calibrate_decision_thresholds(
    ...
    for i in range(ENCODER_LENGTH, T - MAX_HORIZON):
        ...
        c = effective_cost(base_static_cost, gvz_val, session_overlap_flag)
    ...
    def predict(
        ...
        try:
            ...
            if self.calibration is None:
                ...
                c = effective_cost(base_cost, gvz_val, overlap_flag)
...
def train_tft_pro_max(
    ...
    logger.info(f"Training TFT_Pro_Max_V4 on {dev} | {len(df)} bars | epochs={epochs}")
    ...
    # 85% for CV / Training, 15% Strict Holdout for Calibration & Gating
    ...
    logger.info(f"Using Purged Walk-Forward CV (85% Split): {n_folds} folds, Purge={purge_window}, Embargo={embargo_window}")
    ...
    logger.info(f"CV Complete. Avg Val Loss: {avg_cv_loss:.6f}. Training Option A Evaluation Model.")

    # Train Evaluation model strictly up to the holdout boundary
    eval_ds = TFTProMaxDataset(
        observed[:holdout_start], known_future[:holdout_start], static, prices[:holdout_start]
    )
    ...
    logger.info("Executing Holdout Calibration & Promotion Gates.")
    
    # Evaluate Holdout Loss vs Naive Baseline
    df_holdout = df.iloc[holdout_start + warmup - ENCODER_LENGTH:].copy()
    
    holdout_ds = TFTProMaxDataset(
        observed[holdout_start - ENCODER_LENGTH:], 
        known_future[holdout_start - ENCODER_LENGTH:], 
        static, 
        prices[holdout_start - ENCODER_LENGTH:]
    )
    ...
    with torch.no_grad():
        for batch in holdout_loader:
            ...
            for b in range(preds_cpu.shape[0]):
                ...
                c = effective_cost(base_cost, obs_cpu[b, -1, 5], enc_kf_cpu[b, -1, 6])
                costs.append(c)
    ...
    calib = calibrate_decision_thresholds(model_eval, df_holdout, dev, regime)
    coverage_results = validate_quantile_coverage(model_eval, df_holdout, dev, regime=regime)
    
    # Enforce Coverage Gate
    for h in [6, 12]:  # Check key trading horizons
    ...
    # Net PnL Gate & Trade Count Gate
    logger.info("Evaluating Option A Trade Count & Net PnL Promotion Gates...")
    ...
    if total_trades < 50:
        logger.error(f"PROMOTION GATE FAILED: Trade Count Gate. Only {total_trades} trades found on 15% holdout. Minimum 50 required.")
        raise ValueError("Insufficient trade frequency on holdout.")
        
    net_pnl = sum(trades_pnl)
    if net_pnl <= 0:
        logger.error(f"PROMOTION GATE FAILED: Net PnL Gate. Net PnL is {net_pnl:.6f}.")
        raise ValueError("Model has negative expectancy on holdout.")
    ...
    if win_rate < 0.50:
        ...
        raise ValueError("Model win rate is below 50%.")

    logger.info("All Promotion Gates Passed. Executing Option A Direct Promotion (Skipping 100% Retrain).")
    ...
    torch.save({
        ...
        "calibration": calib,  # Strict out-of-sample calibration tied exactly to model_eval weights
        "model_version": "TFT_Pro_Max_V4_OptionA",
        "feature_schema": {
            "required_columns": ["close", "high", "low", "gvz_zscore_20", "dxy", "us10y", "gold_silver_ratio"],
